...
This command will:
    ...
    Prompt for a description of what the agent should do
    Generate an appropriate system prompt and identifier using AI
    Let you select which tools the agent can access
    Let you choose the agent mode (all, primary, or subagent)
...
permission	object	Controls tool access. See Permissions below.
...
Agent configurations are merged from multiple sources. Later sources override earlier ones:
    ...
    Built-in agent defaults (native agents defined in the codebase)
    Global config (~/.config/kilo/config.json)
    Project config (kilo.jsonc in the project root)
    Global agent markdown files (~/.config/kilo/agents/*.md)
    Project agent markdown files (.kilo/agents/*.md)
...
Related
    ...
    Custom Modes — Create specialized primary agents with tool restrictions
